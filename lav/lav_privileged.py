...
from lav.models.rgb import RGBBrakePredictionModel, RGBSegmentationModel
...
class LAV:
    def __init__(self, args):
        ...
        self.bra_model = RGBBrakePredictionModel([4, 10, 18], pretrained=True).to(
            self.device
        )
    ...
    def train_bev(
        ...
        plan_loss = torch.mean(
            F.l1_loss(
                ego_plan_locs,
                ego_locs[:, 1:][:, None, None].repeat(
                    1, self.num_plan_iter, self.num_cmds, 1, 1
                ),
                reduction="none",
            ).mean(dim=[1, 2, 3, 4])[idxs]
            * self.branch_weights[cmds[idxs]]
        )
        ego_cast_loss = (
            F.l1_loss(
        ...
        return dict(
            ...
            ego_plan_locs=_numpy(ego_plan_locs[0, -1, int(cmds[0])])
            * self.pixels_per_meter
            + self.bev_center,
            ego_cast_locs=_numpy(ego_cast_locs[0]) * self.pixels_per_meter
            + self.bev_center,
    ...
    def train_bra(self, rgb1, rgb2, sem1, sem2, bra):
        ...
        loss = (
            F.binary_cross_entropy(pred_bra, bra)
    ...
    def mot_inference(
        ...
        (
            ...
            pred_orimaps,
            pred_bev,
        ) = self.lidar_model(pillar[None], num_point[None], coord[None])
        ...
        ego_plan_locs, other_cast_locs, other_cast_cmds = uniplanner.infer(
            features[0], det[1], cmd, nxp, pixels_per_meter=self.pixels_per_meter
        )
