...
from lav.leaderboard_custom.envs.map_utils import Wrapper as map_utils
from lav.leaderboard_custom.envs.pretty_map_utils import Wrapper as pretty_map_utils
...
class StitchCameraReader:
    def __init__(self, bp_library, vehicle, sensor_spec, reading_frequency=1.0):
        ...
        for prefix, yaw in self.yaws:
            ...
            sensor = CarlaDataProvider.get_world().spawn_actor(
                bp, carla.Transform(sensor_location, sensor_rotation), vehicle
            )
    ...
    def on_camera_func(weakself, prefix):
        def on_camera(event):
            ...
            self.datas[prefix] = (
                array[..., 2] if "semantic" in self.sensor_type else array[..., :3]
            )
        ...
        while self._run_ps:
            if self._callback is not None:
                ...
                if current_time - latest_time > (1 / self._reading_frequency) or (
                    ...
                    self._callback(
                        GenericMeasurement(self.stitched, GameTime.get_frame())
                    )
                    latest_time = GameTime.get_time()
                    first_time = False
                else:
...
class CollisionReader:
    def __init__(self, bp_library, vehicle, reading_frequency=1.0):
        ...
        self.sensor = CarlaDataProvider.get_world().spawn_actor(
            bp, carla.Transform(), vehicle
        )
        self.sensor.listen(
            lambda event: self.__class__.on_collision(weakref.ref(self), event)
        )
        ...
        while self._run_ps:
            if self._callback is not None:
                ...
                if current_time - latest_time > (1 / self._reading_frequency) or (
                    ...
                    self._callback(
                        GenericMeasurement(self._collided, GameTime.get_frame())
                    )
                    latest_time = GameTime.get_time()
                    first_time = False
                else:
...
class ObjectsReader(BaseReader):
    ...
    def _get_speed(self, actors_list):
        return [
            np.linalg.norm([v.get_velocity().x, v.get_velocity().y, v.get_velocity().z])
            for v in actors_list
        ]
...
class SpeedometerReader(BaseReader):
    ...
    def _get_red_light(self, transform=None):
        ...
        light_id = (
            self._vehicle.get_traffic_light().id
            if self._vehicle.get_traffic_light() is not None
            else -1
        )
        lights_list = (
            CarlaDataProvider.get_world().get_actors().filter("*traffic_light*")
        )
