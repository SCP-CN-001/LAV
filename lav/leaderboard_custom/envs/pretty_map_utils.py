...
def is_within_distance_ahead(
    target_transform, current_transform, max_distance, degree=90.0
):
    """
...
def cart2polar(dx, dy):
    ...
    return arctan + {(1, 1): 0, (-1, 1): 180, (-1, -1): 180, (1, -1): 360}.get(
        (dx / abs(dx), dy / abs(dy))
    )
...
class Util:
    ...
    def get_bounding_box(actor, offset_x=0, offset_y=0, scale_x=1, scale_y=1):
        ...
        corners = [
            carla.Location(x=offset_x, y=offset_y) + x + actor.trigger_volume.location
            for x in corners
        ]
...
def get_edge_angle(wp1, wp2):
    ...
    cv1 = np.array(
        [np.cos(yaw1) * np.cos(pitch1), np.sin(yaw1) * np.cos(pitch1), np.sin(pitch1)]
    )
    ...
    cv2 = np.array(
        [np.cos(yaw2) * np.cos(pitch2), np.sin(yaw2) * np.cos(pitch2), np.sin(pitch2)]
    )
...
class MapImage:
    def __init__(
        ...
        self.big_solid_lane_surface = pygame.Surface(
            (width_in_pixels, width_in_pixels)
        ).convert()
        self.big_broken_lane_surface = pygame.Surface(
            (width_in_pixels, width_in_pixels)
        ).convert()
        self.big_stop_surface = pygame.Surface(
            (width_in_pixels, width_in_pixels)
        ).convert()

        self.big_waypoint_surfaces = [
            pygame.Surface((width_in_pixels, width_in_pixels)).convert()
            for _ in range(6)
        ]
        ...
        if recompute:
            self.draw_road_map(
                ...
                self.big_solid_lane_surface,
                self.big_broken_lane_surface,
                self.big_stop_surface,
                self.big_waypoint_surfaces,
                carla_world,
                carla_map,
                self.world_to_pixel,
            ...
            self.big_solid_lane_surface = pygame.image.load_extended(
                f"{save_path}/solid_lane.png"
            )
            self.big_broken_lane_surface = pygame.image.load_extended(
                f"{save_path}/broken_lane.png"
            )
            self.big_stop_surface = pygame.image.load_extended(f"{save_path}/stop.png")
            for i in range(6):
                self.big_waypoint_surfaces[i] = pygame.image.load_extended(
                    f"{save_path}/waypoint{i}.png"
                )
        ...
        if recompute:
            ...
            pygame.image.save_extended(
                self.big_solid_lane_surface, f"{save_path}/solid_lane.png"
            )
            pygame.image.save_extended(
                self.big_broken_lane_surface, f"{save_path}/broken_lane.png"
            )
            pygame.image.save_extended(self.big_stop_surface, f"{save_path}/stop.png")
            for i in range(6):
                pygame.image.save_extended(
                    self.big_waypoint_surfaces[i], f"{save_path}/waypoint{i}.png"
                )
        ...
        self.road_map = np.swapaxes(
            pygame.surfarray.array3d(self.big_map_surface), 0, 1
        )
    ...
    def draw_road_map(
        ...
        def draw_lane_marking(solid_lane_surface, broken_lane_surface, waypoints):
            ...
            draw_lane_marking_single_side(
                solid_lane_surface, broken_lane_surface, waypoints[0], -1
            )

            # Right Side
            draw_lane_marking_single_side(
                solid_lane_surface, broken_lane_surface, waypoints[1], 1
            )

        def draw_lane_marking_single_side(
            solid_lane_surface, broken_lane_surface, waypoints, sign
        ):
            ...
            for markings in markings_list:
                if markings[0] == carla.LaneMarkingType.Solid:
                    draw_solid_line(
                        solid_lane_surface, markings[1], False, markings[2], 2
                    )
                elif markings[0] == carla.LaneMarkingType.Broken:
                    # draw_broken_line(surface, markings[1], False, markings[2], 2)
                    draw_solid_line(
                        broken_lane_surface, markings[1], False, markings[2], 2
                    )
        ...
        def draw_topology(carla_topology, index):
            ...
            for waypoints in set_waypoints:
                ...
                if not waypoint.is_junction:
                    draw_lane_marking(
                        solid_lane_surface, broken_lane_surface, [waypoints, waypoints]
                    )
        ...
        if self.show_connections:
            ...
            for wp in carla_map.generate_waypoints(dist):
                ...
                for nxt, deviation, next_cross in zip(
                    next_wps, deviations, next_crosses
                ):
                    ...
                    if np.min(np.linalg.norm(intersections - [wx, wy], axis=1)) > 0:
                        ...
                        (deviation > threshold)
                        or (next_cross == min(next_crosses) and len(next_wps) > 1)
                    ):
                        map_surfaces = [
                            waypoint_surfaces[RoadOption.LEFT.value - 1],
                            waypoint_surfaces[RoadOption.LANEFOLLOW.value - 1],
                        ...
                        while prev.is_junction:
                            ...
                            for cur_candidate in prev.next(dist):
                                deviation, next_cross = get_edge_angle(
                                    prev, cur_candidate
                                )
                        ...
                        (deviation > threshold)
                        or (next_cross == max(next_crosses) and len(next_wps) > 1)
                    ):
                        map_surfaces = [
                            waypoint_surfaces[RoadOption.RIGHT.value - 1],
                            waypoint_surfaces[RoadOption.LANEFOLLOW.value - 1],
                    ...
                    for map_surface in map_surfaces:
                        pygame.draw.line(
                            map_surface, col, to_pixel(wp), to_pixel(nxt), 1
                        )

                if wp.lane_change & carla.LaneChange.Right:
                    # print ('LANE CHANGE RIGHT!')
                    r = wp.get_right_lane()
                    map_surface = waypoint_surfaces[
                        RoadOption.CHANGELANERIGHT.value - 1
                    ]
                    if r and r.lane_type == carla.LaneType.Driving:
                        ...
                        if len(nexts) > 0:
                            r = nexts[0]
                            pygame.draw.line(
                                map_surface, col, to_pixel(wp), to_pixel(r), 1
                            )
                if wp.lane_change & carla.LaneChange.Left:
                    ...
                    if l and l.lane_type == carla.LaneType.Driving:
                        ...
                        if len(nexts) > 0:
                            l = nexts[0]
                            pygame.draw.line(
                                map_surface, col, to_pixel(wp), to_pixel(l), 1
                            )
...
class ModuleWorld:
    ...
    def start(
        self, show_triggers=False, show_connections=True, show_spawn_points=False
    ):
        # Create Surfaces
        ...
        self.hero_solid_lane_surface = pygame.Surface(
            (scaled_original_size, scaled_original_size)
        ).convert()
        self.hero_broken_lane_surface = pygame.Surface(
            (scaled_original_size, scaled_original_size)
        ).convert()
        self.hero_stop_surface = pygame.Surface(
            (scaled_original_size, scaled_original_size)
        ).convert()
        ...
        self.window_waypoint_surfaces = [
            pygame.Surface((MAP_SIZE, MAP_SIZE)).convert() for _ in range(6)
        ]
    ...
    def _render_traffic_lights(
        self, surface, list_tl, world_to_pixel, world_to_pixel_width
    ):
        self.affected_traffic_light = None
    ...
    def _render_walkers(self, surface, list_w, world_to_pixel, world_to_pixel_width):
        ...
        for w in list_w:
            ...
            if (
                not self._is_point_on_sidewalk(world_pos, world_to_pixel)
    ...
    def render(self, display):
        ...
        if self.hero_actor is not None:
            ...
            self.hero_solid_lane_surface.blit(
                self.map_image.solid_lane_surface, (-offset[0], -offset[1])
            )
            self.hero_broken_lane_surface.blit(
                self.map_image.broken_lane_surface, (-offset[0], -offset[1])
            )
            ...
            [
                surface.blit(m_surface, (-offset[0], -offset[1]))
                for surface, m_surface in zip(
                    self.hero_waypoint_surfaces, self.map_image.waypoint_surfaces
                )
            ]

            # Rotate
            rotated_map_surface = pygame.transform.rotozoom(
                self.hero_map_surface, angle, 1.0
            ).convert()
            rotated_solid_lane_surface = pygame.transform.rotozoom(
                self.hero_solid_lane_surface, angle, 1.0
            ).convert()
            rotated_broken_lane_surface = pygame.transform.rotozoom(
                self.hero_broken_lane_surface, angle, 1.0
            ).convert()
            rotated_vehicle_surface = pygame.transform.rotozoom(
                self.hero_vehicle_surface, angle, 1.0
            ).convert()
            rotated_walker_surface = pygame.transform.rotozoom(
                self.hero_walker_surface, angle, 1.0
            ).convert()
            rotated_waypoint_surfaces = [
                pygame.transform.rotozoom(surface, angle, 1.0).convert()
                for surface in self.hero_waypoint_surfaces
            ]

            center = (display.get_width() / 2, display.get_height() / 2)
            rotation_map_pivot = rotated_map_surface.get_rect(center=center)
            rotation_solid_lane_pivot = rotated_solid_lane_surface.get_rect(
                center=center
            )
            rotation_broken_lane_pivot = rotated_broken_lane_surface.get_rect(
                center=center
            )
            ...
            rotation_waypoint_pivots = [
                surface.get_rect(center=center) for surface in rotated_waypoint_surfaces
            ]

            self.window_map_surface.blit(rotated_map_surface, rotation_map_pivot)
            self.window_solid_lane_surface.blit(
                rotated_solid_lane_surface, rotation_solid_lane_pivot
            )
            self.window_broken_lane_surface.blit(
                rotated_broken_lane_surface, rotation_broken_lane_pivot
            )
            ...
            [
                w_surface.blit(r_surface, pivot)
                for w_surface, r_surface, pivot in zip(
                    self.window_waypoint_surfaces,
                    rotated_waypoint_surfaces,
                    rotation_waypoint_pivots,
                )
            ...
            self.hero_solid_lane_image = make_image(
                self.window_solid_lane_surface
            ).mean(axis=-1)
            self.hero_broken_lane_image = make_image(
                self.window_broken_lane_surface
            ).mean(axis=-1)
            self.hero_vehicle_image = make_image(self.window_vehicle_surface).mean(
                axis=-1
            )
            self.hero_walker_image = make_image(self.window_walker_surface).mean(
                axis=-1
            )
            self.hero_waypoint_images = [
                make_image(surface).mean(axis=-1)
                for surface in self.window_waypoint_surfaces
            ]
        else:
            # Translation offset
            translation_offset = (
...
class Wrapper:
    ...
    def is_point_on_sidewalk(cls, pos):
        return cls.world_module._is_point_on_sidewalk(
            pos, cls.world_module.map_image.world_to_pixel
        )
