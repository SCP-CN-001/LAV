            ...
            for waypoint in topology:
                ...
                for w in waypoints:
                    ...
                    if r is not None and (
                        r.lane_type == carla.LaneType.Shoulder
                        or r.lane_type == carla.LaneType.Parking
                    ):
                ...
                shoulder_left_side = [
                    lateral_shift(w.transform, -w.lane_width * 0.5) for w in shoulder
                ]
                shoulder_right_side = [
                    lateral_shift(w.transform, w.lane_width * 0.5) for w in shoulder
                ]
                ...
                draw_lane_marking(lane_surface, shoulder, False)
                ...
                draw_lane_marking(lane_surface, shoulder, True)

                # Draw Lane Markings and Arrows
                if not waypoint.is_junction:
                    draw_lane_marking(lane_surface, waypoints, True)
                    draw_lane_marking(lane_surface, waypoints, False)
...
class ModuleWorld:
    ...
    def _render_traffic_lights(
        ...
        for tl in list_tl:
            ...
            corners = Util.get_bounding_box(
                tl, offset_x=0.5, offset_y=1.5, scale_x=1.2, scale_y=scale_y
            )
            corners = [world_to_pixel(p) for p in corners]
    ...
    def _render_walkers(self, surface, list_w, world_to_pixel, world_to_pixel_width):
        ...
        for w in list_w:
            ...
            if (
                ...
                pos_x, pos_y = world_to_pixel(
                    world_pos
                    + carla.Location(
                        x=self.hero_x * np.cos(angle + math.pi),
    ...
    def _render_vehicles(self, surface, list_v, world_to_pixel):
        ...
        for v in list_v:
            if (
                "role_name" in v[0].attributes
                and v[0].attributes["role_name"] == "hero"
            ):
                continue

            ignore = False

            fairly_close = is_within_distance_ahead(
                v[0].get_transform(), self.hero_actor.get_transform(), 25, 150
            )
            very_close = is_within_distance_ahead(
                v[0].get_transform(), self.hero_actor.get_transform(), 10, 120
            )
            ...
            if wpt.is_junction:
                ...
                if wpt.road_id != ego_wpt.road_id or wpt.lane_id != ego_wpt.lane_id:
                    passed = True
                    cur_wpt = ego_wpt
                    for _ in range(5):
                        for next_wpt in cur_wpt.next(2.0):
                            if next_wpt.lane_change:
                                continue
                            if (
                                next_wpt.road_id == wpt.road_id
                                and next_wpt.lane_id == wpt.lane_id
                            ):
                                passed = False
                                break
                        if not passed:
            ...
            if hasattr(v[0], "bounding_box"):
                ...
                corners = [
                    carla.Location(
                        x=-bb.x - self.hero_x - 4, y=-bb.y - self.hero_y / 2 - 1.0
                    ),
                    carla.Location(
                        x=-bb.x - self.hero_x - 4, y=bb.y + self.hero_y / 2 + 1.0
                    ),
                    carla.Location(x=bb.x + 0.8, y=bb.y + self.hero_y / 2 + 1.0),
                    carla.Location(x=bb.x + 0.8, y=-bb.y - self.hero_y / 2 - 1.0),
                ]
            else:
                corners = [
    ...
    def render(self, display):
        ...
        if self.hero_actor is not None:
            ...
            self.hero_stop_surface.blit(
                self.map_image.stop_surface, (-offset[0], -offset[1])
            )
            ...
            self.hero_waypoint_images = [
                make_image(surface) for surface in self.hero_waypoint_surfaces
            ]
