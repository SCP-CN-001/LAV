...
class RGBModel(nn.Module):
    ...
    def forward(self, rgb):
        embd = self.backbone(self.normalize(rgb / 255.0))
...
class RGBSegmentationModel(nn.Module):
    def __init__(self, seg_channels):
        ...
        self.normalize = lambda x: (x / 255.0 - 0.5) * 2
...
class RGBBrakePredictionModel(nn.Module):
    ...
    def forward(self, rgb1, rgb2, mask=False):
        x1 = self.conv_backbone(self.normalize(rgb1 / 255.0))
        x2 = self.conv_backbone(self.normalize(rgb2 / 255.0))
