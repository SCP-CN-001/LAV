...
class BEVPlanner(nn.Module):
    def __init__(
        ...
        self.offset_x = nn.Parameter(
            torch.tensor(x_offset).float(), requires_grad=False
        )
        self.offset_y = nn.Parameter(
            torch.tensor(y_offset).float(), requires_grad=False
        )
    ...
    def forward(self, bev, ego_locs, locs, oris, nxps, typs):
        ...
        if int(typs.float().sum()) > 0:
            ...
            # Flatten the locs
            flat_bev = (
                bev.expand(N, *bev.size()).permute(1, 0, 2, 3, 4).contiguous()[typs]
            )
            ...
            other_embd = torch.zeros(
                (N, self.num_out_feature), dtype=dtype, device=device
            )
            other_bev_embd = torch.zeros(
                (N, self.num_out_feature), dtype=dtype, device=device
            )
            ...
            other_cast_cmds = torch.zeros(
                (N, self.num_cmds), dtype=dtype, device=device
            )
    ...
    def _plan(self, embd, nxp, cast_locs, pixels_per_meter=4, crop_size=96):
        ...
        for i in range(self.num_cmds):
            u = torch.cat(
                [u0.expand(self.num_plan, B, -1).permute(1, 0, 2), cast_locs[:, i]],
                dim=2,
            )
    ...
    def crop_feature(
        self, features, rel_locs, rel_oris, pixels_per_meter=4, crop_size=96
    ):
        ...
        rot_x_offset = (
            -k * self.offset_x * cos + k * self.offset_y * sin + self.offset_x
        )
        rot_y_offset = (
            -k * self.offset_x * sin - k * self.offset_y * cos + self.offset_y
        )
        ...
        grids = F.affine_grid(
            theta, torch.Size((B, C, crop_size, crop_size)), align_corners=True
        )
