...
from lav.leaderboard_custom.data_collect_L20.planner import RoutePlanner
from lav.utils import visualize_obs
...
def get_absolute_path(path_):
    path_workspace = os.environ.get("WORKSPACE", None)
    if not os.path.isabs(path_):
        return os.path.join(path_workspace, path_)
    else:
        return path_
...
class AutopilotCollector(ExpertAgent):
    ...
    def flush_data(self):
        if self.log_wandb:
            wandb.log(
                {
                    "vid": wandb.Video(
                        np.stack(self.vizs).transpose((0, 3, 1, 2)),
                        fps=20,
                        format="mp4",
                    )
                }
        ...
        with lmdb_env.begin(write=True) as txn:
            # Trajectory length
            txn.put(b"len", str(length).encode())
            txn.put(b"town", self.town_name.encode())
    ...
    def run_step(self, input_data, timestamp=None):
        ...
        if self.waypointer is None:
            self.waypointer = Waypointer(self._global_plan, gps, pop_lane_change=False)
        ...
        lidar = (
            lids if self.prev_lidar is None else np.concatenate([lids, self.prev_lidar])
        )
        ...
        # Save data
        if (
            self.num_frames % (self.num_repeat + 1) == 0
            and self.stop_count < MAX_STOP
            and not np.isnan([wx, wy]).any()
        ):
