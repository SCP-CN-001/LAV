...
class Waypointer:
    ...
    def tick(self, gnss):
        ...
        for i, (wx, wy, cmd) in enumerate(self.global_plan):
            ...
            if (
                self.checkpoint[2] == RoadOption.LANEFOLLOW
                and cmd != RoadOption.LANEFOLLOW
            ):
            ...
            if distance < threshold and i - self.current_idx == 1:
                ...
                break
            if (
                curr_distance > self._threshold_max
                and distance < threshold
                and i > self.current_idx
                and cmd in [RoadOption.LEFT, RoadOption.RIGHT]
                and self._pop_turning
            ):
                self.checkpoint = (wx, wy, cmd)
                self.current_idx = i
                break
