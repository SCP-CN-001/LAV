...
class LAV:
    def __init__(self, args):
        ...
        self.lidar_model = LiDARModel(
            num_input=len(self.seg_channels) + self.num_frame_stack + 10
            if self.point_painting
            else self.num_frame_stack + 10,
        ...
        bev_planner = BEVPlanner(
            ...
            x_offset=0,
            y_offset=1 + self.min_x / ((self.max_x - self.min_x) / 2),
        ...
        params = (
            list(self.uniplanner.plan_gru.parameters())
        ...
        self.seg_mask = self.build_seg_mask(
            h=H,
            w=W,
            cx=self.bev_center[0],
            cy=self.bev_center[1],
        ).to(self.device)
    ...
    def train_lidar(
        ...
        (
            ...
            pred_orimaps,
            pred_bev,
        ) = self.lidar_model(lidars, num_points)

        (
            other_next_locs,
            other_cast_locs,
            other_cast_cmds,
            other_cast_locs_expert,
            other_cast_cmds_expert,
            ego_next_locs,
            ego_plan_locs,
            ego_cast_locs,
            ego_cast_cmds,
            ego_cast_locs_expert,
            ego_plan_locs_expert,
        ) = self.uniplanner(
            features,
            bev,
            ego_locs,
        ...
        seg_loss = (
            torch.mean(
                F.binary_cross_entropy(pred_bev, seg_bev, reduction="none")
                * self.seg_mask
            )
            * self.seg_weight
        )
        ...
        plan_loss = torch.mean(
            F.l1_loss(
                ego_plan_locs,
                ego_plan_locs_expert[:, -1]
                .gather(1, cmds.expand(self.num_plan, 2, 1, -1).permute(3, 2, 0, 1))
                .unsqueeze(1)
                .repeat(1, self.num_plan_iter, self.num_cmds, 1, 1),
                reduction="none",
            ).mean(dim=[1, 2, 3, 4])
            * self.branch_weights[cmds]
        )
        ...
        if self.distill:
            ...
            ego_cast_loss = (
                F.l1_loss(
                    ego_cast_locs.gather(
                        1, cmds.expand(self.num_plan, 2, 1, -1).permute(3, 2, 0, 1)
                    ).squeeze(1),
                    ego_locs[:, 1:],
                    reduction="none",
                )
            ...
            other_cast_losses = F.l1_loss(
                other_cast_locs,
                other_next_locs.unsqueeze(1).repeat(1, self.num_cmds, 1, 1),
                reduction="none",
            ).mean(dim=[2, 3])
            other_cast_loss = other_cast_losses.min(1)[0].mean()
            cmd_loss = F.binary_cross_entropy(
                ego_cast_cmds,
                (1.0 - self.cmd_smooth) * F.one_hot(cmds, self.num_cmds)
                + self.cmd_smooth / self.num_cmds,
            )

        mot_loss = (
            plan_loss
            + ego_cast_loss
            + other_cast_loss * self.other_weight
            + cmd_loss * self.cmd_weight
        )
        ...
        # inference
        det = self.det_inference(
            torch.sigmoid(pred_heatmaps[0]),
            pred_sizemaps[0],
            pred_orimaps[0],
            break_tie=True,
        )
        ...
        return dict(
            ...
            det=det,
            gt_det=gt_det,
            ego_plan_locs=_numpy(ego_plan_locs) * self.pixels_per_meter
            + self.bev_center,
            ...
            other_cast_locs=_numpy(other_cast_locs) * self.pixels_per_meter
            + self.bev_center,
    ...
    def det_inference(self, heatmaps, sizemaps, orimaps, **kwargs):
        ...
        for i, c in enumerate(heatmaps):
            ...
            for s, x, y in extract_peak(c, **kwargs):
                ...
                if (
                    i == 1
                    and w < 0.1 * self.pixels_per_meter
                    or h < 0.2 * self.pixels_per_meter
                ):
                    continue
    ...
    @torch.no_grad()
    def mot_inference(
        self, lidar, num_point, cmd, nxp, heatmaps=None, sizemaps=None, orimaps=None
    ):
        ...
        (
            ...
            pred_orimaps,
            pred_bev,
        ) = self.lidar_model(lidar[None], num_point[None])
        ...
        ego_plan_locs, _, other_cast_locs, other_cast_cmds = uniplanner.infer(
            features[0], det[1], cmd, nxp
        )
